...
# def _list_rgb_images(folder: Path):
#     return [path for path in sorted(folder.iterdir()) if path.is_file() and path.suffix.lower() in VALID_SUFFIXES]
...
def main(
        data_root: str = '/mnt/data1/ssy/render_people/fill-light-dataset/infer/huawei',
        output_root: Optional[str] = '/mnt/data1/ssy/render_people/fill-light-dataset/infer/huawei/intrinsics',
    ...
    # no iter, just get all files in person_dir
    rgb_images = [path for path in root.glob("*.*")]
    ...
    for rgb_path in rgb_images:
        ...
        robedo_path = output_root_path / rgb_name
        ...
        processed += 1
        print(f'{root} -> {rgb_path} -> {robedo_path}')
