...
class FillLightingSimpleFolderDataset(Dataset):
    def __init__(self, root_dir, image_size=512):
        ...
        if not self.items:
            raise ValueError(
                "No matching samples found across fill/rgb/depth/lighting params folders."
            )
...
def get_dataloaders(
    ...
    validation_dataset = FillLightingSimpleFolderDataset(
        root_dir=validation_data_root, image_size=image_size
    )
...
def main(
    ...
    run_name = create_run_name("Fill-Lighting-Simple")
