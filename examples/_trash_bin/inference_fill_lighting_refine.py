...
def _prepare_pairs(source_path: str, rgb_path: str) -> List[dict]:
    source = Path(source_path)
    rgb = Path(rgb_path)

    if source.is_dir() or rgb.is_dir():
        if not (source.is_dir() and rgb.is_dir()):
            raise ValueError("When using folder inputs, source and rgb must both be folders.")
        source_files = _list_images(source)
        rgb_files = _list_images(rgb)
        if not (source_files and rgb_files):
            raise ValueError("One or more input folders are empty.")
        if len(source_files) != len(rgb_files):
            raise ValueError("Folder inputs must contain the same number of images for pairing.")
        return [
            {
...
def run_inference(config: dict) -> None:
    ...
    for item in pairs:
        ...
        output_name = f"{item['source'].stem}_fill_lighting_refine.png"
...
def main():
    ...
    required_keys = ["source_image", "rgb_image"]
    if not all(merged.get(key) for key in required_keys):
        raise ValueError("source_image and rgb_image must be provided via args or config.")
