...
def _prepare_pairs(
    ...
    lighting_scale = Path(lighting_scale_path) if lighting_scale_path else None

    if albedo.is_dir() or rgb.is_dir() or depth.is_dir():
        if not (albedo.is_dir() and rgb.is_dir() and depth.is_dir()):
            raise ValueError(
                "When using folder inputs, albedo, rgb, and depth must all be folders."
            )
        albedo_files = _list_images(albedo)
        rgb_files = _list_images(rgb)
        depth_files = _list_images(depth)
        if not (albedo_files and rgb_files and depth_files):
            raise ValueError("One or more input folders are empty.")
        if not (len(albedo_files) == len(rgb_files) == len(depth_files)):
            raise ValueError(
                "Folder inputs must contain the same number of images for pairing."
            )
        lighting_scale_files = (
            _list_images(lighting_scale) if lighting_scale and lighting_scale.is_dir() else []
        )
        if lighting_scale_files and len(lighting_scale_files) != len(albedo_files):
            raise ValueError("lighting_scale folder must match albedo count.")
        return [
            {
                "albedo": albedo_files[i],
                "rgb": rgb_files[i],
                "depth": depth_files[i],
                "lighting_scale": lighting_scale_files[i] if lighting_scale_files else None,
            }
            for i in range(len(albedo_files))
        ]
    ...
    return [
        {
            ...
            "rgb": rgb,
            "depth": depth,
            "lighting_scale": lighting_scale if lighting_scale and lighting_scale.exists() else None,
        }
    ]
...
def run_inference(config: dict) -> None:
    ...
    pairs = _prepare_pairs(
        config.get("albedo_image"),
        config.get("rgb_image"),
    ...
    for item in pairs:
        albedo_tensor = _load_rgb(item["albedo"], image_size)
        rgb_tensor = _load_rgb(item["rgb"], image_size)
        depth_tensor = _load_gray(item["depth"], image_size)
        lighting_scale_tensor = (
            _load_gray(item["lighting_scale"], image_size)
            if item.get("lighting_scale") is not None
            else torch.ones_like(depth_tensor)
        )
        shading_tensor = (rgb_tensor / albedo_tensor.clamp(1e-3, 1.0)).clamp(0.0, 1.0)
        batch = {
            config.get("source_key", "source"): (albedo_tensor * 2 - 1)
            .unsqueeze(0)
            .to(device),
        ...
        output_name = f"{item['albedo'].stem}_fill_lighting.png"
...
def main():
    ...
    parser.add_argument("--albedo_image", type=str, default=None)
    ...
    required_keys = ["albedo_image", "rgb_image", "depth_image"]
    if not all(merged.get(key) for key in required_keys):
        raise ValueError(
            "albedo_image, rgb_image, and depth_image must be provided via args or config."
        )
