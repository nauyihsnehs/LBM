...
def run_inference(config: dict) -> None:
    ...
    model = build_relight_model(
        ...
        unet_input_channels=int(config.get("unet_input_channels", 8)),
    ...
    robedo_device = config.get("robedo_device", device)
    robedo_base_size = int(config.get("robedo_base_size", 384))
    robedo_model = AlbedoInference(device=robedo_device, base_size=robedo_base_size)
...
def main():
    ...
    parser.add_argument("--robedo_device", type=str, default=None)
    parser.add_argument("--robedo_base_size", type=int, default=None)
